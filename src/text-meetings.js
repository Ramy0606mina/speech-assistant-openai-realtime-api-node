...
export class TextMeetings {
  ...
  async handle({ text, owner, requestKey, receivedAt, history = [], maxReplyLength = Infinity, allowImplicitConflictConfirmation = true }) {
    ...
    const pending = tail.length && tail.some(item => item.role === 'assistant' && /(?:No invitation was sent|(?:Teams|In-person) proposal:|CONFIRM MEETING|overlaps an existing calendar item|have not prepared or sent another invitation)/i.test(item.content || ''))
    ...
    if (pending && /^confirm meeting[.!]?$/i.test(text)) {
      const proposal = [...tail].reverse().find(item => item.role === 'assistant' && /^(?:Teams|In-person) proposal:/.test(item.content || '') && /CONFIRM MEETING [a-f0-9]{12}/i.test(item.content));
    ...
    if (pending && !wantsMeeting(text) && !/^confirm meeting\b/i.test(text)) {
      if (/\b(?:cancel|forget|stop|never mind|nevermind)\b/i.test(text)) {
        for (const item of tail.filter(item=>item.role==='assistant' && /^(?:Teams|In-person) proposal:/.test(item.content || ''))) {
      ...
      const locationPending=/^Where should the in-person meeting take place/.test(recent.at(-1)?.content||'') && !/^(?:what|why|read|check|search|show)\b/i.test(text);
      if (!locationPending && !/@|\b(?:tomorrow|today|monday|tuesday|wednesday|thursday|friday|saturday|sunday|minutes?|hours?|am|pm|attendee|subject|title|instead|understand|confused|confirm|yes|set|book|schedule|teams|in-person)\b|\d{1,2}:\d{2}/i.test(text)) return null;
    ...
    try {
      ...
      if (/\b(?:phone meeting|zoom|google meet)\b/i.test(text)) return 'Please clarify whether you want an in-person or Microsoft Teams meeting. No invitation was sent.';
  ...
  async prepare(text, owner, requestKey, receivedAt, maxReplyLength, confirmedConflictChange = false) {
    ...
    const modes=[...text.matchAll(/\bin[- ]person\b|\bteams\b/gi)];
    const inPerson=Boolean(modes.length && /^in[- ]person$/i.test(modes.at(-1)[0]));
    const location=inPerson?String(args.location||'').trim():'Microsoft Teams';
    if(inPerson && (!location || location.length>300 || !text.toLowerCase().includes(location.toLowerCase())))return 'Where should the in-person meeting take place? No invitation was sent.';
    const proposal={title:args.title.trim(),startIso:args.startIso,timezone:args.timezone||'America/Toronto',durationMinutes:args.durationMinutes,attendees:[...new Set(attendees)],onlineMeeting:!inPerson,location,body:''};
  ...
  async confirm(code,owner,requestKey,maxReplyLength) {
    ...
    const reply=`${p.onlineMeeting?'Teams':'In-person'} invitation submitted: ${p.title}\n${preview.localStart.replace('T',' ')}–${preview.localEnd.slice(11)} ${p.timezone}\nTo: ${p.attendees.join(', ')}\n${p.onlineMeeting?'See the calendar invitation for the Teams link.':'Location: '+p.location+'.'} Acceptance is not yet confirmed.`;
    ...
    if(!result.id || !result.invitationsSubmitted || (p.onlineMeeting && !result.joinLinkCreated))throw Error('Meeting not verified.');
