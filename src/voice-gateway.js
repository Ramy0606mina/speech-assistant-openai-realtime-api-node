...
export function voiceTools() {
  return [
    ...
    },
    {
      ...
      parameters:{type:'object',properties:{mailbox:{type:'string',enum:['principal','london']},message_id:{type:'string'},to:{type:'array',items:{type:'string'},maxItems:10},subject:{type:'string'},body:{type:'string',description:'Polished plain-text email with a punctuated greeting, blank lines between short complete paragraphs, and a professional closing on its own line. Do not add a sender name or signature block; Ramy adds it when reviewing the draft.'}},required:['body'],additionalProperties:false},
    },
    {
      type: 'function',
      name: 'check_calendar',
