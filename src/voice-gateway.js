...
export function realtimeInstructions() {
  return [
    ...
    'Every email draft must be polished plain text: begin with a recipient greeting and punctuation, use blank lines between short complete paragraphs, use normal sentence punctuation, and end with a professional closing on its own line. Do not add Ramy Mina or any signature block because London appends Ramy’s stored Outlook signature when saving. Do not submit a one-paragraph block. Use bullets only when Ramy asks for a list.',
...
export function voiceTools() {
  return [
    ...
    },
    {
      ...
      parameters:{type:'object',properties:{mailbox:{type:'string',enum:['principal','london']},message_id:{type:'string'},to:{type:'array',items:{type:'string'},maxItems:10},subject:{type:'string'},body:{type:'string',description:'Polished plain-text email with a punctuated greeting, blank lines between short complete paragraphs, and a professional closing on its own line. Do not add a sender name or signature block; London appends the stored Outlook signature.'}},required:['body'],additionalProperties:false},
    },
    {
      type: 'function',
      name: 'check_calendar',
