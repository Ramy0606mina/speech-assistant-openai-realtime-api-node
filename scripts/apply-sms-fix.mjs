...
const activeSender = `const sendTwilioChannelMessage = async ({ to, from, body }) => {
...
if (source.includes(disabledSender)) {
  source = source.replace(disabledSender, activeSender);
}

const inertRoute = "fastify.all('/incoming-sms', async (_request, reply) => reply.type('text/xml').send('<Response/>'));";
const activeRoute = `const handleIncomingSms = async (request, reply) => {
  ...
  console.log('SMS SECURITY CHECK:', {
...
if (source.includes(inertRoute)) {
  source = source.replace(inertRoute, activeRoute);
}
...
if (!source.includes("fastify.all('/incoming-sms', handleIncomingSms);")) {
  throw new Error('SMS route patch did not apply.');
}
if (!source.includes('const sendTwilioChannelMessage = async ({ to, from, body })')) {
  throw new Error('SMS sender patch did not apply.');
}
if (source.includes("SMS is paused and WhatsApp has been removed.")) {
  throw new Error('Disabled SMS sender remains in index.js.');
}
